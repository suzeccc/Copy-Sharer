...
| 图片转文字 | 在 Windows、macOS 和 Linux 本机粘贴图片进行 OCR，不上传云端 |
...
> [!NOTE]
> 图片转文字在 Windows 使用系统 OCR，在 macOS 使用 Apple Vision，在 Linux 使用随安装包提供的 Tesseract 中英文快速模型；各安装包只包含当前平台需要的 OCR 后端。
...
- 点击粘贴区域后按 `Ctrl+V`（macOS 使用 `Cmd+V`），可粘贴截图、位图或复制的图片文件。
- 图片在本机完成预处理和 OCR，不会发送到云端。
- 显示图片预览、原始尺寸、识别状态和字符数量。
- 识别结果可以直接编辑、一键复制或清空。
- 可以再次粘贴新图片替换当前识别会话。
- Windows 使用系统语言包，macOS 使用 Apple Vision，Linux 内置简体中文和英文快速模型。
...
7. 进入「图片转文字」，点击粘贴区域并按 `Ctrl+V`（macOS 使用 `Cmd+V`）识别截图文字。
...
- Windows 请确认已安装所需识别语言包；Linux 请确认使用正式安装包且模型资源完整。
- 图片过大时先缩小或裁剪后重试。
