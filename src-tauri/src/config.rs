...
fn migrate_config(config: &mut AppConfig) -> bool {
    ...
    if config.config_version < 10 {
        if config.max_send_file_size_mib == LEGACY_DEFAULT_FILE_SIZE_LIMIT_MIB {
            config.max_send_file_size_mib = MAX_FILE_SIZE_LIMIT_MIB;
        }
        if config.max_receive_file_size_mib == LEGACY_DEFAULT_FILE_SIZE_LIMIT_MIB {
...
mod tests {
    ...
    #[test]
    fn version_nine_migration_raises_only_legacy_default_file_limits() {
        let mut config = AppConfig {
            config_version: 9,
            max_send_file_size_mib: 2048,
            max_receive_file_size_mib: 1024,
            ..AppConfig::default()
        };
