...
pub async fn handle_file_offer(
    ...
    let manager = manager();
    match manager.handle_offer(app, offer).await {
        Ok(task) => {
            if clipboard_sync {
...
fn validate_transfer_limits(file_count: usize, sizes: &[u64]) -> AppResult<()> {
    ...
    if total > MAX_TRANSFER_TOTAL_SIZE {
        return Err(AppError::InvalidInput(
            "transfer total size cannot exceed 10 GiB".to_string(),
        ));
    }
    Ok(())
}
...
mod tests {
    ...
    fn multi_file_limits_reject_too_many_files_and_oversize_total() {
        ...
        assert!(validate_transfer_limits(1, &[10 * GIB]).is_ok());
        assert!(validate_transfer_limits(1, &[10 * GIB + 1]).is_err());
        assert!(validate_transfer_limits(2, &[6 * GIB, 4 * GIB]).is_ok());
        assert!(validate_transfer_limits(2, &[6 * GIB, 4 * GIB + 1]).is_err());
    }
